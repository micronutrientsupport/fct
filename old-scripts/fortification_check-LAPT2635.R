...
FCT <- read_csv(here::here('data', 'FCT_10.csv')) # This are 8FCTs for SSA + uFish + uPulses (see old scripts)
...
FCT %>% filter(str_detect(fooditem, 'Rice|rice')) %>%
  ggplot(aes(FCT, ZN)) + geom_boxplot()

FCT %>% filter(str_detect(fooditem, '|Wheat|wheat')) %>%
  ggplot(aes(FCT, ZN)) + geom_boxplot()
