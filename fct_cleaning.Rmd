...
Also, it would allow us to run quality check such as, recalculation of variables, SOP, etc. Within the quality checks we will find count of missing values per MN of interest and sum of proximates (SOP) calculated as the sum of water, protein, fat, available carbohydrates, dietary fibre (or total carbohydrates), alcohol (if applicable) and ash. And, percentage of sum of proximates above or below recommeded thresholds (FAO/INFOODS, 2013). For more details see 'quality_check.Rmd'.
...
Additionally, we oils in this FCT has value NA, according to FAO/INFOODS Ash can be calculated as sum of minerals:
