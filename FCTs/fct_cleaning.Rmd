...
Most of the FCTs used here were freely available in the [FAO/INFOODS website](https://www.fao.org/infoods/infoods/tables-and-databases/en/), and particuarly under [FAO/INFOODS:Africa](https://www.fao.org/infoods/infoods/tables-and-databases/africa/en/). 


In section 5 (Quality checks), we provide information on drawbacks and usage warnings  (i.e., food items to be avoided) for each FCT.
...
#### Standtadisation of variables using FAO/INFOODS tagname
...
### 1) Western Africa FCT (2019)

**Version log**
V.2.0.3 updated on 2023/08/04 - Bug fix: Changed dictionary code for WA19("02_041") from "23170.01.02" to "23170.01.03", and WA19("02_040") from "23170.01.03" to "23170.01.02", change WA19(07_030) from "21121.01" to "21121.04". 
V.2.0.2 updated on 2023/06/20 - Bug fix: Changed dictionary code for WA("07_046") from "21116.01" to "21116.03". 
V.2.0.1 updated on 2023/05/31 - Bug fix: Updated one dictionary code for WA19("02_004") from "23170.01" to "23170.01.01", and WA19("06_018") from "1379.02" to "1379.02.01". 
V.2.0.0 updated on 2023/05/19 - Important changes in dictionary codes, bug fixed on values that were reported with brackets, and combining Tagnames for fat, vitamin B6 and thiamine.
...
It can be downloaded from [here](http://www.fao.org/fileadmin/user_upload/faoweb/2020/WAFCT_2019.xlsx), using the following piece of code. Depending on the system, you need to use "curl" for OS X / Linux, "wininet" for Windows.

```{r eval = FALSE}

file <- "http://www.fao.org/fileadmin/user_upload/faoweb/2020/WAFCT_2019.xlsx"

download.file(file, "./FCTs/WA19/INFOODS-WAFCT_2019.xlsx",
               method="wininet", 
               mode="wb")
...
#### 2. Harmonising the data

##### 2.2. Special characters

Currently, we are, when possible, adding values within the bracket to a new column, or updating an existing if it was empty. Then, we are keeping the values in the table in the same column, this is to help with minimising the missing values when doing food matching. 

#TBD: Adding that information to the metadata "comment" column. 

- Below is point the history of thoughts around it. 
...
Rice, white, raw (01_037) was assigned to imported rice (23161.01.02), mainly because it is for the whole region hence we assumed that it would be an average for all countries. However, because we are not 100% sure, we have assigned confidence low.  (**TO BE REVIEW**). We used parboiled because the "white rice" was assigned to the "polished" it was decided based on the fibre content. 
rice, white, polished (fibre = 1.9g/100g) == rice, white 
rice, white (fibre = 3.6g/100g) == rice, parboiled

**Meat**

- WA19(07_030): Changed chicken WA19(07_030) to a more detail code, as when we calculate the fat content of the "unespecified" chicken in KE18 was the mean of the fat content with skin in WA19. 
...
**Version log**
V.2.0.2 updated on 2023/06/20 - Bug fix: Changed dictionary code for MW19("MW03_0052") from "21116.01" to "21116.03", as it is the same fat concentration as "moderately fat" in WA19. 
V.2.0.1 updated on 2023/05/31 - Bug fix: Updated one dictionary code for MW19("MW01_0017") from "23170.01" to "23170.01.01", for MW19("MW08_0007") from "1802.01" to  "1802.02".
V.2.0.0 updated on 2023/05/19 - New standardisation of scripts, using excel version of the FCT, and combining Tagnames for fat, vitamin B6 and thiamine.
...
#### MAPS dictionary updates

1. Changed sugar cane to "sugar cane juice" due to NVs (i.e., water and Energy) were the same as the juice when compared with the values in KE18. 
...
2. Adding "african cake" as banana cake: https://cookpad.com/uk/recipes/13097874-truly-african-banana-cake-chigumu

## Crop composition data 

Updated 2024-07-28

Malawi crop composition data

Trying to get the water content of some of the missing values.

```{r}

 mwi_mn %>% filter(is.na(water)) 

```


**Mucuna**: We used the velvet bean dried water content. 


### 3) Kenya FCT

**Version log**
V.2.1.0 updated on 2023/08/07 - New variable: Generate the ALCg variable. Bug fix: Updated one dictionary code for KE18("15125") back to the original "F0022.08", KE18(9007) from "21641.01.01" to "21641.02.01". 
V.2.0.2 updated on 2023/07/10 - Bug fix: Updated one dictionary code for KE18("5006") from "1319.05" to "1319.08".
V.2.0.1 updated on 2023/05/31 - Bug fix: Updated one dictionary code for KE18("15125") from "F0022.08" to "F0022.07", KE18(4018) from  "21393.9.05" to "1290.9.15". 
V.2.0.0 updated on 2023/05/19 - New standardisation of scripts, combining Tagnames for fat and thiamine. 
...
- The scientific name of pineapple (*Ananas comosus*) is missing ("5030"). 

**2) Typos and misspellings**

- There is a typo in "Roti" which was spelled with two "t"s. We re-named to "Roti (Indian Chapati)". 

**3) Incomplete information or description**

Salt, ionised KE18(13027), is reported as fortified with I, however, KE18 is not reporting I content in foods. 
...
Then tagnames were standardized following the standard names as described above and on FCT_QA, some decision point are described below: 
 ...
 ) Edible portion coefficient: EDIBLE (fraction, no units). According to the FCT documentation description, the edible account for total food as purchased. For instance, edible portion of cod fillet is 0.26, as it is from whole fish to fillet. 
...
) Alcohol: It was not reported as a variable in the FCT. In the documentation, it is reported as 

>
"Alcohol: Alcohol is assumed to be zero in all foods except alcoholic beverages. For these foods, the alcohol content is indicated in the food name."

Therefore, we assumed ALCg = 0, and then, for the only three foods with supposed alcohol content, only 3 alcoholic beverages: (12007) Wine, Red (9.4 % alcohol),  (12008) Wine, White, Dry (10.3 % alcohol), and (12009) Wine, White, Sweet (10.2 % alcohol). The alcohol percentage was reported in the food description only in the pdf version on page 136. We used that information to calculate the alcohol concentration (g/100g), by using the equation provided in the Guidelines (ref-INFOODS)

Eq. XX () ALCg = ALC(%)(v/v)*0.789(g/mL)/density(g/mL)

Where 0.789g/mL is the alcohol density, and we need the density of each of the foods that were obtained form the Table of densities (Ref-INFOODS). For item 12007, we used the density of "Wine, table, red" = 0.99g/100mL (line 154), for 12008, we used gravity of "Wine, white, dry or sparkling" = 0.995g/mL (line 185), and, similarly for 12009, "Wine, white, sweet" = 1.015g/mL (line 187). 
...
The food item "13002 - Baking powder, dry powder" has quite extreme Ca (and other minerals) values. In addition, it has a very high standard deviation. It was not an analytical value and all the sources of the nutrient values were from abroad (see `r kenfct$biblio_id[kenfct$code == "13002"]`). Furthermore, even being used in small amounts, it was the main driver of Ca consumption in IHS4 and IHS5. Hence, we are not using this item, and hence removing the dictionary code from it. However, the value is plausible for that kind of product as it can contain calcium carbonate. 
...
Mustard seed oil (9007) was wrongly coded as "rape seed oil" (21641.01.01). Now it is correclty changed to mustard seed oil (21641.02.01).

- **Fruits**: We used the same dictionary code for custard apple (sweet sop) KE18("5006") as for soursop which are the same family as **. However they are different foods, and they were reported individually in SL18. Hence, we changed to code from "1319.05" to "1319.08" (See change log).
...
**Version log**
V.1.5.1 updated on 2023/05/18 - Changing the unit of cholesterol (CHOLE) from g to the standard unit mg, and excluding one beer from dict & replacing Energy in kcal with NA for alcoholic beverages, and fixing a typo in folate (FOLmg -> FOLmcg). 
...
#### 1. Obtaining the data (Data Access)

The excel file used in this repository can be downloaded [here](https://www.fao.org/fileadmin/templates/food_composition/documents/LesFCT_final_copyrevised1.xlsm)
...
1. This FCT do not report alcohol content in foods yet it has two alcoholic beverages, see food group "BEVERAGES, ALCOHOLIC". We advise to avoid the use of these foods for nutrient assessment. 
...
**Version log**
v.3.0.0 updated on 2023/07/10 - Incorrect variable - fixed CHOALVMg to CHOALVg
V.2.0.2 updated on 2023/07/10 - Bug fixed: Excluded the item "17-692" due to high number of missing values.
...
The data can be obtained from the UK website [here]().

All the steps of the cleaning can be found in the report [here](), and it was done as part of the project (xxx), and the original repository can be found [here]()
...
Food components are occasionally expressed using different units of measurement (e.g., g of Ca per 100g of fresh weight, edible portion or mg of Ca per 100g of fresh weigh, edible portion) which are standardised (including denominators) following the FAO/INFOODS Guidelines for Converting Units, Denominators and Expression (FAO/INFOODS, 2012).

**CHOALVg**: Carbohydrates were expressed as "Available Carbohydrates expressed as monosaccharide equivalents" (CHOALVM) and we needed to change it to "Available Carbohydrates expressed by weight" (CHOALV) in order for them to be comparable with "Available Carbohydrates by difference (CHOALVDg). To do so, we were using the recommendation from the UK21 - user guide (REF) (page 16) of diving CHOALVMg by 1.05. Although the FAO/INFOODS recommended the use of the fractions, in most of the food entries the fractions were not always available. 
...
-**17-692**: We excluded the use of popcorn because it has many missing values, particularly for micronutrients. 

- **17-734**: We are using ovaltine fortified (23999.01.01) because the values seemed in line with the ovaltine fortied reported in WA19(13_028). 
...
The data can be obtained from the US FoodCentral website [here]().
...
The seasoning mix: US19(2074, 2075, 2076) have high number of missing values, and values for Vit. A were quite high, hence we are excluding those foods.  

###### MAPS Food dictionary 

- **23999.01.01**: We are using ovaltine fortified for US19(14315) because the values seemed in line with the ovaltine fortified reported in WA19(13_028).
 - **F0623.08**: We used the item US19(9100) (heavy syrup) from the four options available because the water content was the closest match to the water content in our "reference value" WA19(05_034).
- **F0623.07**: We used the item US19(9271) (extra heavy syrup) from the options available because the water content was the closest match to the reference food KE18(5029).
- **F1232.31**: We selected item US19(21108) from the options available because the water content was the closest match to the reference food UK21(19-544).
- **2920.01**: We opted to match US19(90560) to this dictionary code because the water content was the same as the "snail" in WA19(07_083), which has the same dictionary code.
- **23120.03.03**: We opted for US19(20020) instead of US19(20016) because the protein content was more similar to other maize yellow items in the SSA FCTs.
...
# Food components standardisation 

There are few food components that needed unit transformation 

We are standardisation some food components: 

### Niacin - "nia_conversion_std"   
...
There are some variables that we want to keep but we are not goin g to rename. The fastest approach is to proceed as with KENFCT and cut into two part. Then, to modify separately both and then bind them. 
